...
struct ThoughtNode {
    ...
    float confidence;
    uint64_t timestamp;
    std::vector<NodeID> involved_nodes;
...
public:
    ...
    std::vector<NodeID> get_recent_context() const {
    ...
    float compute_context_overlap(const std::vector<NodeID>& nodes) const {
...
struct AttentionState {
    std::unordered_map<NodeID, float> attention_weights;
    std::vector<std::unordered_map<NodeID, float>> attention_history;
    float total_entropy = 0.0f;
    int depth = 0;
    
    void update_attention(const std::unordered_map<NodeID, float>& new_weights) {
    ...
    std::vector<NodeID> get_top_nodes(int k = 3) const {
        std::vector<std::pair<NodeID, float>> sorted_nodes;
        for (const auto& [node, weight] : attention_weights) {
