    ...
    for (const auto& vis_event : visual_events) {
        ...
        for (size_t i = 0; i < audio_node_ids.size(); ++i) {
            if (impl_->within_temporal_window(vis_event.timestamp, audio_timestamps[i])) {
                // Create cross-modal edge (EXACT - co-occurrence in time)
                graph.add_edge(vis_node_id, audio_node_ids[i], RelationType::EXACT, 1.0f);
    ...
    for (const auto& curr : current_events) {
        ...
        for (const auto& prev : previous_events) {
            ...
            if (curr_node != prev_node) {
                // Create temporal edge (LEAP - inferred temporal relationship)
                graph.add_edge(prev_node, curr_node, RelationType::LEAP, 0.5f);
    ...
    for (size_t i = 0; i < node_ids.size(); ++i) {
        for (size_t j = i + 1; j < node_ids.size(); ++j) {
            if (node_ids[i] != node_ids[j]) {
                // EXACT edge (spatial co-occurrence = direct observation)
                graph.add_edge(node_ids[i], node_ids[j], RelationType::EXACT, 1.0f);
