...
enum class Rel : uint32_t {
    ...
    ISA = 4,            // 100 - "X is/are Y" relationships
    CONSUMES = 5,       // 101 - "X drinks/eats Y" relationships
    HAS = 6,            // 110 - "X has Y" relationships
    CAN = 7,            // 111 - "X can Y" relationships
    MAX_REL = 7
};
...
// Hash function specialization for std::array<uint8_t, 32> (used by NodeID, EdgeID, PathID)
namespace std {
    template<>
    struct hash<std::array<uint8_t, 32>> {
        size_t operator()(const std::array<uint8_t, 32>& id) const noexcept {
            size_t h = 0;
            for (size_t i = 0; i < id.size(); ++i) {
                h ^= std::hash<uint8_t>{}(id[i]) + 0x9e3779b9 + (h << 6) + (h >> 2);
            }
            return h;
        }
    };
    ...
    template<>
    struct hash<std::pair<std::array<uint8_t, 32>, std::array<uint8_t, 32>>> {
        size_t operator()(const std::pair<std::array<uint8_t, 32>, std::array<uint8_t, 32>>& p) const noexcept {
            size_t h1 = hash<std::array<uint8_t, 32>>{}(p.first);
            size_t h2 = hash<std::array<uint8_t, 32>>{}(p.second);
