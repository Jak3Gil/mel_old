...
public:
    ...
    /**
     * Get attention dynamics (for querying boredom, etc.)
     */
    const AttentionDynamics& get_dynamics() const { return dynamics_; }
    
private:
    AtomicGraph& graph_;
    AttentionDynamics dynamics_;  // NEW: Anti-sticking mechanisms
    uint64_t current_focus_;
    float current_focus_score_;
    int frames_on_current_;  // Track staring duration
